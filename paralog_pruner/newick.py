...
"""Class for working with Newick objects.
"""
...
def _read_label(label, node, interior_node):
    """ Takes a node label, current working node and a boolean, signifying
    whether if the label belongs to an interior node or not, as an input.
    Adds the information to the current node, or a new node, depending on
    if it is an interior node or not.
    """
    label = label.split(')')[0]
    if ':' in label:
...
def verify_newick(nw_str):
    """ Takes a Newick string as an input and performs a bunch of tests to
